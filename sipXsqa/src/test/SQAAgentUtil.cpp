...
void SQAAgentUtil::generateSQAAgentData(std::vector<SQAAgentData::Ptr>& agents, unsigned int agentsNum, bool ha)
{
  ...
  for (unsigned int i = 0; i < agentsNum; i++)
  {
    ...
    data->ha = ha;
    data->userApp = SQAUserAppGeneric;
...
void SQAAgentUtil::setUserAppPreferredAddress(SQAAgentData::Ptr data, SQAClientUserApp userApp, const std::string& preferredAddress)
{
  data->userApp = userApp;
  data->sqaControlPreferredAddress = preferredAddress;
}
...
void generateSQACClientIni(SQAAgentData::Ptr data, bool noLocalSQAAgent)
{
  ...
  if (data->userApp == SQAUserAppRLS)
  {
    ofs << "sqa-control-address-rls=" << data->sqaControlPreferredAddress << "\n";
  }
  else if (data->userApp == SQAUserAppProxyPlugin)
  {
    ofs << "sqa-control-address-proxy-plugin=" << data->sqaControlPreferredAddress << "\n";
  }
